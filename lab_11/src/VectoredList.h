...
public:
    ...
    public:
        ...
        [[maybe_unused]]string get()const{return buck_point->get_exact_element(cursor);};
...
public:
    ...
    VectoredList(VectoredList &&to_be_copied) noexcept;
    ~VectoredList();
    ...
    VectoredList& operator=(VectoredList&& to_be_copied)noexcept;
    ...
    [[maybe_unused]]void erase(const VectoredListIterator& to_delete_beg,
               const VectoredListIterator& to_delete_end);
    VectoredListIterator begin()const;
    VectoredListIterator end()const;
    void assign(vector<string>& to_be_assigned);
};
